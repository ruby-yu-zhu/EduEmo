...
class AUX_FGM():
    ...
    def adversarial(self,aux=False):
    ...
    def adversarial_training(self, args, inputs, device, optimizer, aux=False):
        self.backup_param_data()
        self.adversarial(aux)
        loss = self.model(inputs, device)[0]
        loss_backward(args, loss, optimizer)
        self.restore_param_data()


class AUX_PGD():
    def __init__(self, model, param_name, alpha=0.5, epsilon=1.0, K=3):
        self.model = model
        self.param_name = param_name
        self.alpha = alpha
